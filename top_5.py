...
def load_data(existing_file=None):
    if existing_file:
        ...
        required_columns = ['Headline', 'Description', 'Category']
...
# Display Top 5 News Articles Section
st.markdown("<h2 style='color: #2980B9;'>Top 5 News Articles</h2>", unsafe_allow_html=True)

# Assuming 'Real_Prob' is used to rank the news articles
df_sorted = df.sort_values(by='Real_Prob', ascending=False).head(5)
...
for index, row in df_sorted.iterrows():
    st.markdown(
        ...
        <div class="top-news-box">
            <div class="top-news-title">{row['Headline']}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
